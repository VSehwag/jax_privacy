...
class GuassianDiffusion:
    ...
    def __init__(self, timesteps: int, dim: int = 4):
        ...
        self.scalars = self.get_all_scalars(self.alpha_bar_scheduler,
                                            self.timesteps)

        self.clamp_x0 = lambda x: x.clip(-1, 1)
        self.get_x0_from_xt_eps = lambda xt, eps, t, scalars: (
            self.clamp_x0(1 / jnp.sqrt(scalars.alpha_bar[t]) *
                          (xt - jnp.sqrt(1 - scalars.alpha_bar[t]) * eps)))
        self.get_pred_mean_from_x0_xt = (
            lambda xt, x0, t, scalars:
            (jnp.sqrt(scalars.alpha_bar[t]) * scalars.beta[t]) / (
            ...
            all_scalars["beta"] = jnp.asarray([
                min(
                    ...
                    0.999,
                       ...
                       (self.dim - 1))  # hardcoding beta_max to 0.999
        else:
    ...
    def sample_from_forward_process(self,
                                    x0: np.ndarray,
                                    t: np.ndarray,
                                    rng: jnp.ndarray,
                                    eps=None):
    ...
    def sample_from_reverse_process(
        ...
        alpha_bar_div = jnp.pad(alpha_bar.ravel(), [1, 0],
                                constant_values=1.0)[:-1].reshape(
                                    alpha_bar.shape)
        new_betas = 1 - (alpha_bar / alpha_bar_div)
        scalars = self.get_all_scalars(self.alpha_bar_scheduler, timesteps,
                                       new_betas)
        ...
        for i, t in zip(np.arange(timesteps)[::-1], new_timesteps[::-1]):
            ...
            pred_eps, state = net.apply(params,
                                        state,
                                        None, {
                                            "x": final,
                                            "t": current_t,
            ...
            pred_x0 = self.get_x0_from_xt_eps(final, pred_eps, current_sub_t,
                                              scalars)
            pred_mean = self.get_pred_mean_from_x0_xt(final, pred_x0,
            ...
            if i == 0:
                ...
                if ddim:
                    final = (
                        jnp.sqrt(scalars["alpha_bar"][current_sub_t - 1]) *
                        pred_x0 +
                        jnp.sqrt(1 - scalars["alpha_bar"][current_sub_t - 1]) *
                        pred_eps)
                else:
                    final = pred_mean + jnp.sqrt(
                        scalars.beta_tilde[current_sub_t]) * jax.random.normal(
                            key=rng, shape=final.shape)
        return final, state

    def sample_from_reverse_process_lax(
            self,
            net,
            params: hk.Params,
            state: hk.State,
        ...
        def body_fn(final, t):
            i = jnp.argwhere(new_timesteps == t, size=1)[0][0]
            current_t = jnp.asarray([t] * len(final))
            current_sub_t = jnp.asarray([i] * len(final))

            pred_eps, _ = net.apply(params,
                                    state,
                                    None, {
                                        "x": final,
                                        "t": current_t,
            ...
            if ddim:
                final = (
                    jnp.sqrt(scalars["alpha_bar"][current_sub_t - 1]) * pred_x0
                    + jnp.sqrt(1 - scalars["alpha_bar"][current_sub_t - 1]) *
                    pred_eps)
            else:
                final = pred_mean + jnp.sqrt(
        ...
        final, _ = jax.lax.scan(body_fn, xT, new_timesteps[::-1])
